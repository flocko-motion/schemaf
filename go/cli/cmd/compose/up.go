...
func newUpCmd(ctx *cli.Context) *cobra.Command {
	var devMode string
...
Examples:
  atlas compose up atlas-graph/compose/graph-api.yml
  atlas compose up atlas-graph/compose/graph-api.yml --dev db,core
  atlas compose up atlas-graph/compose/graph-api.yml --native core`,
	...
	cmd.Flags().StringVar(&devMode, "dev", "", "Run only these services in Docker (comma-separated short names)")
